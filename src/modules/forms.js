...
const addLabelInput = (form, lbl, type, name, required) => {
  let label = setElemWithAttrs('label', [['for', name]], capitalize(lbl));
...
const addPriorityList = (form, name) => {
  let label = setElemWithAttrs('label', [['for', 'Priority']], 'Priority');
...
const saveTask = () => {
  if (tasks) {
    let task = tasks.find(t => t.id === getValue('t-id'));
...
const saveProject = () => {
  if (projects) {
    let project = projects.find(p => p.id === getValue('p-id'));
